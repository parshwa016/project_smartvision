    ...
    <div className="min-h-screen bg-slate-100 flex flex-col font-sans overflow-x-hidden">
      ...
      <div className="relative flex-grow flex flex-col items-center pt-32 pb-20 px-6">
        <div className="absolute inset-0 z-0">
          ...
          <div className="absolute inset-0 bg-gradient-to-b from-purple-900/25 via-slate-900/45 to-slate-900/90" />
        </div>
        ...
        <div className="relative z-10 max-w-5xl w-full mx-auto">
          ...
          <div className="mb-12 text-center md:text-left">
            <h1 className="text-5xl md:text-7xl font-black text-white drop-shadow-2xl italic tracking-tighter uppercase mb-2">
              Memory <span className="text-purple-300">Match</span>
            </h1>
            <p className="text-lg md:text-xl text-white/85 font-bold drop-shadow-md flex items-center gap-2 justify-center md:justify-start flex-wrap">
              <Brain className="text-amber-300 shrink-0" size={22} />
              Brain power game — learn first, then play rounds!
              <Sparkles className="text-yellow-300 shrink-0" size={22} />
            </p>
          </div>

          <div className="grid grid-cols-1 md:grid-cols-2 gap-10 max-w-5xl mx-auto">
            <div className="group bg-white rounded-[2.5rem] shadow-2xl overflow-hidden flex flex-col border-4 border-white hover:border-purple-300 transition-all duration-300">
              <div className="relative aspect-[16/10] overflow-hidden">
                <img
                  src={learnThumb}
                  alt="Memory match learning"
                  className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-700"
                />
                <div className="absolute inset-0 bg-gradient-to-t from-black/80 to-transparent" />
                <h3 className="absolute bottom-4 left-6 text-2xl font-black text-white">Mix &amp; learn</h3>
              </div>
              <div className="p-8 bg-white flex flex-col flex-grow">
                <p className="text-xs font-black uppercase tracking-widest mb-3 text-purple-600 italic">Phase 01</p>
                <p className="text-slate-600 font-bold text-sm mb-8 leading-relaxed">
                  Learn a fun mix: alphabets, animals, fruits, and shapes. Tap to hear and remember!
                </p>
                <button
                  type="button"
                  onClick={() => navigate("/game/memory/learn")}
                  className="mt-auto flex items-center justify-center gap-3 w-full py-4 bg-purple-600 text-white text-lg font-black rounded-2xl shadow-[0_6px_0_0_#6b21a8] active:translate-y-1 active:shadow-none transition-all uppercase"
                >
            ...
            <div className="group bg-white rounded-[2.5rem] shadow-2xl overflow-hidden flex flex-col border-4 border-white hover:border-amber-300 transition-all duration-300">
              <div className="relative aspect-[16/10] overflow-hidden">
                <img
                  src={playThumb}
                  alt="Memory match play"
                  className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-700"
                />
                <div className="absolute inset-0 bg-gradient-to-t from-black/80 to-transparent" />
                <h3 className="absolute bottom-4 left-6 text-2xl font-black text-white">Match &amp; score</h3>
              </div>
              <div className="p-8 bg-white flex flex-col flex-grow">
                <p className="text-xs font-black uppercase tracking-widest mb-3 text-amber-500 italic">Phase 02</p>
                <p className="text-slate-600 font-bold text-sm mb-8 leading-relaxed">
                  Limited rounds — match pairs, earn points, then see your score vs your best with a celebration!
                </p>
                <button
                  type="button"
