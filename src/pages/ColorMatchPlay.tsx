...
import { COLOR_PLAY_POOL, pickChoices, pickRandom, shuffle, type ColorItem } from "../data/colorMatchData";
import {
  bestFromHistory,
  loadColorMatchHistory,
  saveColorMatchAttempt,
  type ColorMatchAttempt,
} from "../utils/colorMatchStorage";
...
function newRound(): Quiz {
  const question = pickRandom(COLOR_PLAY_POOL);
  const choices = pickChoices(COLOR_PLAY_POOL, question, 4);
...
const ColorMatchPlay = () => {
  ...
  const [quiz, setQuiz] = useState<Quiz>(() => {
    // make round 1 predictable: avoid rainbow first sometimes
    const first = shuffle(COLOR_PLAY_POOL.filter((c) => c.id !== "rainbow"));
    const question = first[0] ?? pickRandom(COLOR_PLAY_POOL);
    return { question, choices: pickChoices(COLOR_PLAY_POOL, question, 4) };
  });

  /** Snapshot of history before this session (for “previous” comparison). */
  const [historyBefore] = useState<ColorMatchAttempt[]>(() => loadColorMatchHistory());
  const [historyAfter, setHistoryAfter] = useState<ColorMatchAttempt[] | null>(null);
  ...
  const finishGame = (finalScore: number) => {
    const updated = saveColorMatchAttempt(finalScore, TOTAL_ROUNDS);
    setHistoryAfter(updated);
    setPhase("complete");
    const tier = tierForScore(finalScore, TOTAL_ROUNDS);
    if (tier === "star" || tier === "great") {
      speak(`You are a Color Champion! You scored ${finalScore} out of ${TOTAL_ROUNDS}!`);
    } else if (tier === "good") {
      speak(`Nice! You got ${finalScore} right.`);
    } else {
      speak("Keep practicing! Colors will be easy very soon.");
    }
  };

  const nextRound = (picked: ColorItem) => {
    ...
    if (correct) {
      ...
      speak(`Oops! That was ${picked.name}. The right color is ${quiz.question.name}.`);
    }
          ...
          {phase === "playing" && (
            <div className="bg-white/95 rounded-[2rem] shadow-2xl border-4 border-white p-6 sm:p-8 relative overflow-hidden">
              <div className="flex items-center justify-between mb-4">
                <div className="inline-flex items-center gap-2 text-sky-700 font-black uppercase tracking-widest text-xs">
                  <Play size={18} fill="currentColor" /> 🎮 Phase 2: Play Time
                </div>
              ...
              <div className="mb-4">
                ...
                <div className="h-3 w-full rounded-full bg-slate-200 overflow-hidden border-2 border-slate-300">
                  <div
                    className="h-full rounded-full bg-gradient-to-r from-sky-500 via-emerald-400 to-amber-400 transition-all duration-300 ease-out"
              ...
              <div className="text-center mb-6">
                <p className="font-black text-slate-800 uppercase text-lg mb-3">Match the correct color</p>
                <div className="inline-flex items-center gap-2 bg-slate-100 border-2 border-slate-200 rounded-2xl px-4 py-3">
                  <span className="font-black text-slate-700 uppercase tracking-wide">{quiz.question.name}</span>
                  <span className="text-xl" aria-hidden>
                    {quiz.question.emoji}
                  </span>
                </div>
                <p className="mt-3 text-xs font-bold text-slate-600">
                  Can you find the right color? Let&apos;s try!
                </p>
              </div>
              ...
              <div className="grid grid-cols-2 gap-3">
                {quiz.choices.map((c) => (
                  <button
                    ...
                    type="button"
                    onClick={() => nextRound(c)}
                    className="group rounded-2xl border-4 border-white shadow-lg overflow-hidden hover:scale-[1.02] active:scale-95 transition-all"
                    aria-label={`Pick ${c.name}`}
                  >
                    <div
                      className={`aspect-[4/3] w-full ${c.bgClass ? c.bgClass : ""}`}
                      style={swatchStyle(c)}
                      aria-hidden
                    />
                    <div className="p-3 bg-white flex items-center justify-between gap-2">
                      <span className="font-black uppercase text-sm text-slate-800">{c.name}</span>
              ...
              <p className="mt-5 text-center text-xs font-bold text-slate-600">
                Every correct match makes you a <span className="text-sky-700 font-black">Color Champion</span>!
              </p>
            </div>
          )}
            ...
            <div
              className={`relative rounded-[2rem] shadow-2xl border-4 p-6 sm:p-8 overflow-hidden animate-quiz-pop ${
                tier === "retry"
                  ...
                  : "bg-gradient-to-br from-amber-50 via-white to-sky-50 border-amber-300"
              }`}
            >
              ...
              <div className="relative z-10 text-center">
                ...
                <h2
                  className={`text-3xl sm:text-4xl font-black uppercase tracking-tight mb-2 ${
                    tier === "retry"
                      ? "text-slate-700"
                      : "text-transparent bg-clip-text bg-gradient-to-r from-sky-600 via-emerald-500 to-amber-500"
                  }`}
                >
                  {tier === "star" && "Color Champion!"}
                ...
                {tier === "retry" && (
                  <p className="text-slate-600 font-bold text-sm mb-6 flex items-center justify-center gap-2 animate-[quiz-fade-up_0.5s_ease-out_0.35s_both]">
                    ...
                    Watch the color, read the name, then match!
                  </p>
                )}
